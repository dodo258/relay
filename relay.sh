...
SHORTCUT_PATH="/usr/local/bin/relay"

# 仓库地址
REPO_RAW_URL="https://raw.githubusercontent.com/dodo258/relay/main"
...
_bootstrap() {
    ...
    # 下载入口脚本
    if _download "$REPO_RAW_URL/relay.sh" "$INSTALL_DIR/relay.sh"; then
        chmod +x "$INSTALL_DIR/relay.sh"
        echo -e "  ${_GREEN}✓${_NC} relay.sh"
    else
        echo -e "  ${_RED}✗${_NC} relay.sh 下载失败"
        return 1
    fi
    ...
    # 创建快捷命令
    ln -sf "$INSTALL_DIR/relay.sh" "$SHORTCUT_PATH"
...
_load_libs() {
    if [ ! -d "$LIB_DIR" ] || [ ! -f "$LIB_DIR/core.sh" ]; then
        ...
        echo -e "${_BLUE}wget -qO- ${REPO_RAW_URL}/relay.sh | sudo bash -s install${_NC}"
        return 1
    fi
